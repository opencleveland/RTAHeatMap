...
def single_gmaps_output(addr_lat, addr_lon, stop_lat, stop_lon, 
  ...
  while(done == False):
    try:
      ...
      if times_run > threshold: # We've failed too many times, return dummy data
...
def get_closest_stops_for_each_address(addresses, stops, n_stops):
  ...
  for i, addr in addresses.iterrows():
    ...
    for j, stop in stops.iterrows():
      ...
      # This will store our subsets of n_stops stops for each address. Once
      # we've used it to determine which stop is closest, we will add every
      ...
      for j, pair in pair_df.iterrows():
        ...
        try:
          ...
          # Sometimes newrow is only 12 cols when it needs to be 13. don't know why
		  # This just adds another value to the array.
		  # A better solution may just be to make a one-row dataframe, that way we
		  # can append, and pandas will automatically match the columns up.
          print('Value Error!')
          pair_api_df.loc[gl_index] = (newrow + [False] + [''])
        gl_index += 1
      # Assign True to the 'closest' column of the address-stop pair that
      # has the minimum walking distance for this address
      pair_api_df.loc[pair_api_df['distance_value'].idxmin()]['closest'] = True
